...
def random_test(Number_test,test_city):
    ...
    with open('../random_test/{}/bio_true_{}_result.txt'.format(tag,test_city),'a') as f:
        f.write('{}\n'.format(bio_result))
    with open('../random_test/{}/geo_true_{}_result.txt'.format(tag,test_city),'a') as f:
        f.write('{}\n'.format(geo_rseult))
    return None

inp = [(j,i) for i in ['BER','LON','MAR','PXO','SOF','STO','Kiev', 'Oslo', 'Paris', 'Vienna'] for j in range(1)]
with Pool(72) as p:
