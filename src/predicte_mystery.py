...
for test_city in ['BER','LON','MAR','PXO','SOF','STO','Kiev', 'Oslo', 'Paris', 'Vienna']:
    # test_city = 'PXO'
    ...
    for i in test_pre_proba.index:
        ...
        tmp_map['BIOprob'] = [bio_kriging.predict([b_x,b_y]) for b_x,b_y in tmp_map[['bio_x','bio_y']].values]
        tmp_map['GEOprob'] = [geo_kriging.predict([g_x,g_y]) for g_x,g_y in tmp_map[['x','y']].values]

        if not os.path.isdir('../kriging_result/{}/'.format(tag)):
            os.makedirs('../kriging_result/{}/'.format(tag))
      
        tmp_map[['city_ascii','GEOprob','BIOprob','x','y','bio_x','bio_y']].to_csv('../kriging_result/{}/{}.csv'.format(tag,i.split('_')[-1]),index=False)